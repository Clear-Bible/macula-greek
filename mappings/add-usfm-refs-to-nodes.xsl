...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:function name="macula:USFMVerseRangeRefs" as="item()*">
        <xsl:param name="range"/>
        <xsl:variable name="startCh"
            select="tokenize(substring(tokenize($range, '-')[1], 4), ':')[1]"/>
        <xsl:variable name="endCh" select="tokenize(tokenize($range, '-')[2], ':')[1]"/>
        <xsl:variable name="startV"
            select="tokenize(substring(tokenize($range, '-')[1], 4), ':')[2]"/>
        <xsl:variable name="endV" select="tokenize(tokenize($range, '-')[2], ':')[2]"/>
        <xsl:variable name="startW"
            select="tokenize(substring(tokenize($range, '-')[1], 4), ':')[3]"/>
        <xsl:variable name="endW" select="tokenize(tokenize($range, '-')[2], ':')[3]"/>
        <!-- Return [1] startRef string, and [2] endRef string -->
        <xsl:value-of select="concat($startCh, ':', $startV, '!', $startW)"/>
        <xsl:value-of select="concat($endCh, ':', $endV, '!', $endW)"/>
    </xsl:function>
            ...
            <xsl:attribute name="n"> <!-- @n is an id for alphabetical sorting in canonical order. See discussion at https://github.com/Clear-Bible/symphony-team/issues/38 -->
                <xsl:choose>
                    ...
                    <xsl:when test="$bookNumber &lt; 40">
                        ...
                        <xsl:value-of select="concat('o', ./@morphId)"/>
                    </xsl:when>
                    <!-- TEST FOR LXX BOOKS?
                    <xsl:when test=""> 
    ...
    <xsl:template match="@osisId | @ID | @usfmRef | @word | @verse"/>
